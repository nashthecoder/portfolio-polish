...
export const OutcomesSlide = () => (
  <div className="slide-inner" style={{ background: "linear-gradient(135deg, hsl(241 37% 17%) 0%, hsl(270 30% 20%) 60%, hsl(301 42% 27%) 100%)" }}>
    ...
    <p className="text-foreground/90 text-sm font-light max-w-[680px] mb-6">
    ...
    <div className="flex gap-px bg-foreground/10 rounded-xl overflow-hidden mb-6">
      {outcomes.map(o => (
        <div key={o.label} className="flex-1 bg-foreground/5 p-5 text-center">
          ...
          <div className="stat-label text-foreground/90">{o.label}</div>
        </div>
      ))}
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <p className="font-mono text-[0.65rem] text-foreground/90 uppercase tracking-widest mb-3">Sustainability Features Shipped</p>
      ...
      <div>
        <p className="font-mono text-[0.65rem] text-foreground/90 uppercase tracking-widest mb-3">4 Honest Learnings</p>
        <div className="flex flex-col gap-2">
          {learnings.map(l => (
            <div key={l.title} className={`content-card-dark border-l-[3px] ${l.color}`}>
              <div className="text-sm font-bold mb-0.5">{l.title}</div>
              <div className="text-sm text-foreground/80 leading-snug">{l.desc}</div>
            </div>
          ))}
        </div>
      </div>
